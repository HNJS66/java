...
public class Test {
    ...
    private static String getCollegeName(List<Student> students, int sNumber) {
        return students.stream()
                ...
                .findFirst()
                .flatMap(student -> Optional.ofNullable(student.getTeacher()))
                .map(Teacher::getCollege)
                .map(College::getName)
                .orElse("未知学院");
    }
    ...
    private static void printCollegeName(List<Student> students, int sNumber, int tNumber) {
        students.stream()
                ...
                .ifPresent(student -> {
                    ...
                    if (teacherOptional.isPresent() && teacherOptional.get().getNumber() == tNumber) {
                        String collegeName = getCollegeName(student);
                        System.out.println(collegeName.equals("未知学院") ? "未知学院" : collegeName);
                    } else {
